...
> **交付状态:已实现并提交**——keda 分支 `feat/rv-oracle-gate`(`49274d4`..`56a96dd`,6 commit)+ 模板仓分支 `feat/prd-oracle-evidence-model`(`0d1a06a9`),每个 commit 均过完整 `just test`。下列按**实际交付**勾选;v1 与原计划的差异见 §10 交付差异 / §12 / §13。真实-issue 端到端 seeded-bug 验证列为 §12 跟进项(需真实 LLM verifier,不阻塞合并)。
...
- [x] ④ 门禁契约:v1 采用 **pre-PR 门禁**——"PR 存在 ⟹ verifier 已过",故 **未做** daemon `validation/verifier-passed` 双门禁(pre-PR 放置已蕴含之,见 D-12)。
...
- [x] `rg -n "iar:verifier-verdict" src/backend` 仅命中 `run_verifier_agent.py`;`validation/verifier-passed` label 按 D-12 **未引入**。
...
- FR-5 / FR-6:`validation/verifier-passed` label 与 daemon **双门禁 v1 未做**——verifier 放 **pre-PR**,"PR 存在 ⟹ verifier 已过"已蕴含之(见 D-12)。red 经既有 recovery 自动打回 builder(见 D-11)。
...
- **Follow-Up（v1 增量）**：verifier 改用**全新 worktree@builder_sha**(当前复用 builder worktree)并在其中复跑负控的"弄坏"以确认会红;`yellow` 在 PR 贴**警告评论**(当前仅日志);若将来把 verifier 挪到 post-PR,再补 daemon `validation/verifier-passed` 双门禁。
