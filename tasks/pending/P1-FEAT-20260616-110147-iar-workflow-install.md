...
- [ ] **缺失 `.iar.toml` 守卫真实验证**：在裸临时 Git 仓库直接 `uv run iar workflow install preview`，确认报错信息提示"Run the following command from the repository root"（与 `cli.py:_handle_not_initialized_error` 实际文案一致）。
...
- [ ] 未 import `scripts/provision_preview_server.py`（`rg -n "from scripts|import scripts" src/backend/` 为空）
...
- [ ] `pyproject.toml` 含 `[tool.setuptools] include_package_data = true` 且 `MANIFEST.in` 已声明 `recursive-include src/backend/engines/agent_runner/templates *`，确保 sdist 路径下模板仍可被 `importlib.resources` 发现
- [ ] `[preview]` 占位段字段名派生自 `backend.infrastructure.config.settings.PreviewSettings.model_fields`，IAR 中无硬编码字段清单（`rg -n "base_domain|project_slug" src/backend/engines/agent_runner/workflow_install.py` 应只在 `PreviewSettings` 导入附近出现一次，不应有字段名重复罗列）
...
- [ ] `config.toml` 末尾追加最小化 `[preview]` 段（带 header comment 指向 README）；已存在则跳过；`--force` 同时覆盖已存在文件与 `[preview]` 段
- [ ] 未知 `<name>` 返回 `Unknown workflow: <name>` 并非零退出
- [ ] 缺 `.iar.toml` 时报错并提示 `iar init`，非零退出
- [ ] 收到 `--repo` / `--repo-id` / `--config` 任意一个全局 flag 时，dispatch 层 reject 并非零退出，文件不落盘（FR-12）
- [ ] 当 `config.toml` 无法被 `tomlkit` 解析时，打印 warning 并跳过 `[preview]` 段写入，其余 7 个文件按既定逻辑继续（FR-11）
...
- [ ] 真实入口：未知 name 报错退出
- [ ] 真实入口：`--force` 在已存在 7 文件 + 已存在 `[preview]` 段时同时覆盖两者
- [ ] 真实入口：全局 `--repo` / `--repo-id` / `--config` 任一被 reject 且不落盘
- [ ] 打包验证：`uv build` 后 `importlib.resources.files('backend.engines.agent_runner.templates')` 可枚举模板
- [ ] Drift 门禁：`just check-template-drift` 在工作树与同步源一致时退出 0
...
- [ ] `[preview]` 占位段字段集合（`PreviewSettings.model_fields.keys()`，共 10 个含 `enabled`）与 `deploy/vps-traefik/README.md` 第 35-45 行的字段名集合完全一致
