...
- [ ] **Agent 解析真实验证**：创建含模糊和明确格式描述的 PRD Issue，确认 agent 正确区分 "参考 screenshot 目录"（none）和 "截图留证"（screenshot），物化为正确的 marker。
- [ ] **Runner 消费 marker 真实验证**：对物化了 marker 的 Issue，runner 按 marker 要求检查证据格式，不再误判。
- [ ] **回退路径真实验证**：断网或 agent 超时情况下，Issue 仍能创建，runner 回退到正则匹配。
- [ ] **为什么单元测试不够**：Agent 解析涉及真实的 agent 进程调用（claude/codex）、JSON 输出解析、Issue body 物化；需通过真实 `iar issue create` 端到端验证。
...
- [ ] `create_issue_from_prd` 通过 `IContentGenerator` 端口调用 agent，不直接 import `infrastructure/`。
- [ ] 新增 marker 使用 `<!-- iar:... -->` + 命名捕获组正则，与现有 marker 同型。
- [ ] Agent 调用失败不回滚 Issue 创建，符合 fail-soft 原则。

### Behavior Acceptance

- [ ] Agent 将 "截图留证" 解析为 `kind=screenshot`。
- [ ] Agent 将 "参考 screenshots 目录" 解析为 `kind=none`。
- [ ] Agent 将 "导出 PDF 并核对" 解析为 `kind=pdf`。
- [ ] `extract_evidence_format_markers` 从 Issue body 正确提取 `{1: "screenshot", 2: "txt", 3: "none"}`。
- [ ] 有 marker 时 runner 按 marker 检查，无 marker 时回退正则。
- [ ] Agent 超时/JSON 解析失败时，Issue 创建成功，runner 回退正则。

### Documentation Acceptance

- [ ] `docs/guides/agent-runner.md` 更新证据格式检查机制说明。

### Validation Acceptance

- [ ] `uv run pytest tests/test_agent_runner_validation.py -q` 通过。
- [ ] `just test` 全量通过，无既有用例回归。
- [ ] 带凭据环境完成 Realistic Validation 真实入口验证并记录结果。
