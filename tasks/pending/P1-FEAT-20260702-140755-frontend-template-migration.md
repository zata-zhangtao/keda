...
|---|---|---|---|---|---|
| 1 | frontend-admin/ 导入 + 基线绿 | pending | no | — | 独立包,不触其他前端 |
| 2 | frontend-public/ 导入 + 基线绿 | pending | no | — | 独立包,不触其他前端 |
| 3 | 吸收 keda/frontend 7 页面 + 5 组件到 frontend-public/ + API/cookie 基线 | pending | no | — | 依赖 Slice 1/2 绿;依赖 Slice 3 绿后才能收尾 |
| 4 | Phase 5 收尾:删 frontend/ shared/ + 同步 justfile/docker-compose/docs/mkdocs | pending | no | — | 依赖 Slice 1/2/3 全绿 |
...
1. **高风险 oracle 结果(置顶)**:
   - rv-3 + rv-4 证据:浏览器登录后 Cookie 名严格区分 `session_id` / `admin_session_id`,跨域访问被后端 401 拒绝 —— 两域 cookie 边界正确。
   - rv-5 证据:frontend-public/ 下 6 个受保护页面(dashboard / processes / repositories / stats / roadmap / ideas)逐一 200 渲染,`/api/v1/agent-runner/*` 调用 diff 与原 keda/frontend `shared/api/*` 列表完全一致 —— 7 页面 + 5 组件吸收完整。
   - rv-6 证据:Phase 5 收尾后 `rg` 断言无 `@shared/` 与旧 `frontend/` 残留引用;两个前端 `pnpm build` 全绿;`just test` 全绿。
...
- [ ] 两域 cookie 边界(public `session_id` / admin `admin_session_id`)在两个新前端中正确分流,rv-3 / rv-4 实测通过(§2 唯一人工确认项)。
- [ ] 旧 `frontend/` 与 `shared/` 在收尾阶段删除不可逆操作前已确认 Slice 1/2/3 全基线绿,rv-6 实测通过。

### Architecture Acceptance

- [ ] `frontend-admin/` 与 `frontend-public/` 作为 keda 仓根两个独立 npm 包存在,各自有 `package.json` `pnpm-lock.yaml` 各自的 dev/build/typecheck/lint 脚本。
- [ ] 仓根新增 `pnpm-workspace.yaml` 声明两个 workspace。
- [ ] 不动后端 Python 代码(由 `git diff src/backend/` 为空兜底)。
- [ ] 不动 DB schema(由 `git diff alembic/` 为空 + `git diff src/backend/infrastructure/persistence/` schema 部分为空兜底)。
- [ ] 不为 frontend-public/ 新增 Vitest 测试栈(模板原状无 vitest;由 `frontend-public/package.json` 无 vitest 依赖兜底)。

### Dependency Acceptance

- [ ] `pnpm install` 在 keda 仓根成功(workspace 解析两个 frontend)。
- [ ] frontend-admin/ 沿用模板原状依赖(由 `pnpm-lock.yaml` 存在且 `pnpm install` 无版本冲突兜底)。
- [ ] frontend-public/ 沿用模板原状依赖(同上)。
- [ ] 模板版本 pin 到实施时 `~/code/zata_code_template` 仓 `main` hash 并记录于 Slice 提交说明。

### Behavior Acceptance

- [ ] Slice 1 提交:frontend-admin/ 7 个前端模块(features/* 含 apps/auth/chats/dashboard/errors/projects/settings/tasks/users)文件结构与模板原状一致;`pnpm dev` 启动后 `/login` 200。
- [ ] Slice 2 提交:frontend-public/ marketing/auth/app 三组路由与模板原状一致;`pnpm dev` 启动后 `/` 200,`/login` 200。
- [ ] Slice 3 提交:frontend-public/ (app)/ 下新增 dashboard/processes/repositories/stats/roadmap/ideas 6 个页面,lib/api/ 下 7 个 API 模块(client/auth/agentRunner/console/ideaInbox/roadmap/types),components/agent-runner/ 下 5 个组件,components/auth/ 下 SessionProvider + RequireSession;`/api/v1/agent-runner/*` 调用 diff 与 keda/frontend 原状完全一致;默认 cookie = session_id。
- [ ] Phase 5 提交:`frontend/` 与 `shared/` 已删除;`just frontend` 拆为 `just frontend-public` 与 `just frontend-admin`;`docker-compose.yml` 拆分 frontend 服务;`docs/architecture/system-design.md` 与 `docs/architecture/frontend-architecture.md` 同步;`mkdocs.yml` 导航更新;`README.md` / `README_CN.md` 快速开始段落更新。

### Frontend Acceptance

- [ ] frontend-admin/ `pnpm dev` 启动后浏览器访问 `http://localhost:5173` 返回 200,`/login` 渲染登录表单。
- [ ] frontend-public/ `pnpm dev` 启动后浏览器访问 `http://localhost:3000` 返回 marketing 首页,`/login` 渲染登录表单。
- [ ] 两个 dev server 与 keda 后端(`uv run python -m backend.main` 8000)联通,`/api/v1/*` 代理/rewrite 生效。
- [ ] frontend-public/ 登录后访问 `/dashboard` 等 6 个受保护页面均 200,渲染对应 agent-runner 监控数据。
- [ ] frontend-admin/ 登录后访问 `/dashboard` 200,渲染 admin 后台骨架。

### Documentation Acceptance

- [ ] `docs/architecture/system-design.md` 中 FE/FE_ADMIN/FE_PUBLIC 子图更新为已落地状态,旧 frontend/ 引用清零。
- [ ] `docs/architecture/frontend-architecture.md` 改为索引页(指向 frontend-admin/README.md 与 frontend-public/README.md)或删除(由本 PRD Decision Log D-07 决定)。
- [ ] `mkdocs.yml` 导航中 frontend 相关条目同步更新。
- [ ] `README.md` / `README_CN.md` 快速开始段落包含两个新前端的 dev 命令。

### Validation Acceptance

- [ ] rv-1:frontend-admin/ `pnpm build + lint + typecheck` 全绿 + dev server 启动 + `/` 200 + `/login` 200(命令与输出记录于提交说明)。
- [ ] rv-2:frontend-public/ `pnpm build + typecheck + lint` 全绿 + dev server 启动 + `/` 200 + `/login` 200。
- [ ] rv-3:public 登录后 Cookie 名 = session_id,`/dashboard` 200 渲染(后端真实)。
- [ ] rv-4:admin 登录后 Cookie 名 = admin_session_id,`/dashboard` 200 渲染(后端真实)。
- [ ] rv-5:frontend-public/ 下 6 个受保护页面 + 5 个组件逐一 200 渲染;`/api/v1/agent-runner/*` 调用列表 diff 与原 keda/frontend `shared/api/*` 列表完全一致。
- [ ] rv-6:`test ! -d frontend && test ! -d shared && rg -n 'from .@shared/' . | rg -v 'node_modules|.next/|dist/'` 为空;`rg -n 'frontend/(src|shared|Dockerfile|nginx|index|package|vite|tsconfig)' docs/ justfile justfile.shared docker-compose.yml mkdocs.yml README.md README_CN.md | rg -v 'frontend-admin|frontend-public'` 为空;frontend-public build 0 错;frontend-admin build + lint 0 错;`just test` 全绿。
- [ ] 端到端:在 frontend-public/ 中用真实 public 凭据登录 → 进入 dashboard → 下钻到 issue detail → 执行一次 issue action(读状态),完整流程无报错(无凭据 fallback:跳过实际登录,只验证 dev server 与 marketing/静态资源 200)。

### Delivery Readiness

- [ ] 3 片 strangler 切片全部对基线变绿并按顺序提交。
- [ ] Phase 5 收尾删除 frontend/ + shared/ 位于最后,`rg` 断言无残留。
- [ ] `just test` 全绿。
- [ ] `just lint --full` 通过(含 reuse / architecture / guidelines-consistency / max-file-lines)。
- [ ] 无已知回归或发布阻塞项。
- [ ] 本 PRD 收尾后归档到 `tasks/archive/P1-FEAT-20260702-140755-frontend-template-migration.md`。
