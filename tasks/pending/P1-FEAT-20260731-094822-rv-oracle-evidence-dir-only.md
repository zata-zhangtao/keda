...
| 证据留存策略写在了会被同步覆盖的文件里 | **本 PRD 范围外，但需尽快处理** | 策略本身早已定过：`b310c83`（2026-07-21）确立「`tasks/evidence/` 只提交 `*.verification-plan.md` / `*.evidence-report.md` / `*.verifier-report.md`，原始产物 gitignore」，并配了 `.gitignore` 三行细则（`tasks/evidence/**` + 反排除目录 + 反排除 `*.md`），保住审计报告、丢掉体积与密钥风险。问题在**落点**：它直接改了 `docs/ai-standards/testing.md`，而 `scripts/shared/template/sync_template.sh` 的 `_is_upstream_owned` 明确把 `docs/ai-standards/*` 列为上游所有、"the project should never hand-edit it"——下次 sync 会把这段说明抹掉。已核对上游 `zata-codes-template`：其 `testing.md` 无此段（0 处命中），`.gitignore:62` 只有粗放的 `tasks/evidence/`（连 `.md` 报告一起排除，比 keda 的版本更钝）。freshai 两样都没拿到，因此积下 34 个原始产物（17 png / 14 txt / 2 html / 1 webm）。**处理方向**：把 b310c83 的说明与三行 `.gitignore` 上提到模板仓，再 sync 回 keda 与 freshai，最后 `git rm --cached` 清理 freshai 存量。 |
