...
- 所有模块直接引用 `backend.infrastructure.logging.logger` 的 `logger` 单例。
- `api/` 层通过 `engines/factory.py` 导入 `logger`，符合架构约束。
- `logger` 单例在首次访问时自动配置 console 和 file handler，无需额外初始化函数。
...
- 在 `process_runner.py` 引入 `from backend.infrastructure.logging.logger import logger`。
- **`run_filtered_claude_stream()`**：
  - 终端实时输出保持 `print()`（保证流式体验）。
  - 关键结构化事件（`[agent tool]`、`[agent result]`、`[agent error]`）通过 `logger.info()` 记录，自动获得时间戳和文件落盘。
  - 流式文本增量（text_delta）在 `message_stop` 边界处汇总，通过 `logger.info("Agent output: %s", text_buffer)` 整句记录。
...
- **`SubprocessTranscriptRunner.run()` 与 `_run_agent_with_stdin_prompt()`**（Kimi / Codex 的 deliberation 路径）：
  - 当前 `print(line, end="")` 改为通过辅助函数，同时 `print()` 到终端和 `logger.info()` 到日志。
...
# 从 factory.py 导出 logger，让 api/ 层可以访问
__all__ = ["logger", ...]
```
...
- [x] 所有模块直接使用 `logger` 单例，`api/` 层通过 `factory.py` 导入。
- [x] `logger` 单例在首次访问时自动配置 console 和 file handler。
- [x] `SubprocessRunner.run()` 的非 Claude 路径使用 `subprocess.PIPE` 而非 `stdout=None`。
- [x] `run_filtered_claude_stream()` 的关键事件（tool / result / error）通过 `logger.info()` 记录。
...
- **FR-9**: 所有模块必须直接使用 `logger` 单例，`api/` 层通过 `engines/factory.py` 导入以符合架构约束。
...
| D-09 | logger 引用方式 | 所有模块直接使用 `logger` 单例，从 `factory.py` 导出 | `configure_cli_logging()` 配置 root logger + 各模块用 `logging.getLogger(__name__)` | 直接引用 `logger` 单例更简洁，无需配置 root logger 传播机制；`api/` 层通过 `engines/factory.py` 导入 `logger` 符合架构约束；删除了 `configure_cli_logging()` 函数和 `import logging`。 |
