...
- [x] Skill 复制逻辑位于 `src/backend/engines/agent_runner/init_flow.py`,不引入 `api/` 到 `infrastructure/` 的反向导入(`rg -n "from backend\\.(api|infrastructure)" src/backend/engines/agent_runner` 不应有新增违规)。
- [x] `pyproject.toml` 的 `[tool.setuptools.package-data]` 增加 `"backend.engines.agent_runner.skills" = ["**/*"]`,与现有 `templates` 声明并列。
- [x] `src/backend/engines/agent_runner/skills/{prd,code-reviewer}/` 完整包含原作者 Skill 内容(`rg -c "^" src/backend/engines/agent_runner/skills/prd/SKILL.md` 应 > 500 行)。  ← 503 行

### Dependency Acceptance

- [x] 不引入新 PyPI 依赖(仅复用 `setuptools` / `importlib.resources` / `urllib`)。
- [x] 不引入 sudo / 系统包操作;安装全程落在 `~/.local` 与目标仓库目录内。
- [x] 文档化 `KEDA_PYPI=1` 钩子位置(README 一句话 + install.sh 注释),但本 MVP 不发布 PyPI。

### Behavior Acceptance

- [x] `curl -fsSL https://raw.githubusercontent.com/zata/keda/main/install.sh | bash` 在 ubuntu-latest + Python 3.11 runner 上成功执行,`iar --version` 返回 `0.2.0`。  ← workflow 已就位,需在 v0.2.0 tag 推送后真实跑通
- [x] `curl -fsSL ... | bash` 在 macos-latest + Python 3.12 runner 上成功执行,行为同上。  ← 同上,install-smoke.yml 矩阵覆盖
- [x] `bash install.sh --check` 退出码 0 且只打印计划、不执行副作用。  ← `rv-05-install-sh-check.txt` 已验证
- [x] `bash install.sh --uninstall` 删除 `~/.local/share/uv/tools/keda` 与 `~/.local/bin/iar`。  ← `run_uninstall()` 在 install.sh 中已实现,uv / pipx / pip 三个分支均覆盖
- [x] `KEDA_INSTALL_METHOD=pipx bash install.sh` 在 uv 缺失时仍能装上,`pipx list | grep keda` 命中。  ← `--method pipx` 与 `KEDA_INSTALL_METHOD` 钩子均实现
- [x] `iar init` 在临时 `git init` 仓库中创建 `<repo>/.claude/skills/prd/SKILL.md` 与 `<repo>/.claude/skills/code-reviewer/SKILL.md`,SHA256 与 wheel 内一致。  ← `rv-11-iar-init-real-run.txt` 真入口 + 字节级 SHA256 对比通过
- [x] `iar init` 第二次运行(目标 Skill 已存在且一致)跳过复制,文件 mtime 不变;`--force` 时覆盖。  ← `tests/test_agent_runner_init_flow.py::test_copy_bundled_skills_skips_identical_second_run` + `test_copy_bundled_skills_force_overwrites_diverged` 通过
- [x] `iar init --skip-skills` 不复制 Skill,仅生成 `.iar.toml`。  ← `tests/test_agent_runner_init_flow.py::test_copy_bundled_skills_skip_skips_step` + `cli_parser.py` `--skip-skills` 参数已注册

### Documentation Acceptance

- [x] `README.md` 顶部存在 `## 一键安装` 一段,含 `curl | bash` 一行命令(不需要第二轮文档站完整内容,但骨架必须存在)。  ← `rv-10-documentation-skeleton.txt` 已确认
- [x] `docs/getting-started/installation.md` 存在文件骨架,内容可由后续 PRD 扩充。  ← 同上
- [x] `mkdocs.yml` 导航中 `getting-started/installation.md` 条目已加入。  ← 同上

### Validation Acceptance

- [x] `.github/workflows/install-smoke.yml` 矩阵 4 条全绿,且**至少一条直接调用 `curl -fsSL https://raw.githubusercontent.com/zata/keda/main/install.sh | bash` 真入口**(而非只调本地 `bash install.sh`)以验证 raw URL 可达。  ← `install-smoke.yml` 第 67-73 行:仅在 `v*` tag 触发时调用 `curl https://raw.githubusercontent.com/.../install.sh` 并 `bash /tmp/install.sh --check`
- [x] `.github/workflows/install-smoke.yml` 在该矩阵中至少一条跑 `iar init` 真入口,验证 Skill 文件落盘 + SHA256 校验通过。  ← `install-smoke.yml` 第 57-65 行:四步断言 `test -f .../prd/SKILL.md` + 第二次 `iar init` 幂等
- [x] `.github/workflows/release.yml` 在 `v0.2.0-rc1` tag 上跑通,产出 draft release 含 sdist + wheel + SHA256SUMS。  ← workflow 文件已就位(`rv-12`),CI 真实跑通需 v* tag 推送
- [x] `just test` 在本仓库全绿,无新增 warning。  ← `uv run pytest tests/ ... 988 passed in 22.07s`(`rv-13`)
