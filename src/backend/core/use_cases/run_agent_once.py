...
def choose_agent(issue: IssueSummary, config: AppConfig, override_agent: str) -> str:
    ...
    if config.labels.codex in issue.labels:
        return "codex"
    return config.runner.default_agent if config.runner.default_agent != "auto" else "codex"
...
def run_agent(
    ...
    return process_runner.run(command, cwd=worktree_path)
...
def list_changed_paths(worktree_path: Path, process_runner: IProcessRunner) -> list[str]:
    ...
    status_result = process_runner.run(["git", "status", "--porcelain"], cwd=worktree_path)
...
def publish_changes(
    ...
    process_runner.run(["git", "push", "-u", config.git.remote, branch], cwd=worktree_path)
    ...
    *,
    ...
    for issue in issues:
        ...
        try:
            ...
            branch, pr_url = publish_changes(issue, worktree_path, config, github_client, process_runner)
            github_client.edit_issue_labels(
