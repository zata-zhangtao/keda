...
iar issue create        PRD 含 Realistic Validation 清单
                        → Issue body 物化 "## Realistic Validation" 未勾清单
...
commit 前门禁           要求验证但 .iar/evidence/ 为空 → 进入 recovery，
                        重试耗尽后 agent/failed
...
- PRD 侧规范：Realistic Validation 默认必做；只有 operator 确认的 PRD 才允许在 `### Realistic Validation` 小节写 `Validation Waiver: <理由>` 行，`iar issue create` 会将其物化为豁免 marker。
