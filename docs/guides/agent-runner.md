...
- **issue create**：从 PRD Markdown 文件创建 GitHub Issue，并可在 ready 前发布 PRD
- **run**：单次轮询 `agent/ready` 的 Issues，claim 后执行 AI Agent，验证、pre-push review、创建 draft PR、进入 `agent/supervising` 并运行 post-PR supervisor
- **review**：单次检查 `agent/supervising` 和 `agent/review` 的 Issues，基于 PR 上下文变化运行 supervisor cycle
- **review-daemon**：常驻进程，按指定间隔循环执行 `review`
- **daemon**：常驻进程，按指定间隔循环执行 `run`
...
`iar run` 在实现 agent 完成并提交后、push 之前，会执行一次 pre-push AI code review：
...
`iar recover` 只做发布收尾：校验 worktree 干净、校验分支、push、创建或复用 Draft PR、更新 Issue label 和 comment。它不会运行 pre-push review（因为恢复路径要求本地 commit 已经由正常 runner 路径完成过 pre-push review）。
...
如果失败发生在 `Refusing to publish forbidden paths: ...` 这类 forbidden path 拦截处，并且人工已经确认这些文件可以提交、手动创建了本地 commit，应改走 `agent/running` 的本地 commit 复用路径，让 `iar run` 执行完整的 verification、pre-push review、publish 和 post-PR supervisor：
...
`--publish-prd` 是显式 Git 发布行为，未传入时 `iar issue create` 只创建 Issue 并本地回写 PRD，不执行 `git add`、`git commit` 或 `git push`。
...
# 4. 创建 Issue
uv run --project /path/to/keda iar issue create tasks/pending/xxx.md --agent codex --publish-prd --ready
...
publish                 - diff 混入证据路径 → 拒绝 push（双保险）
                        ...
                        - PR 上发证据评论（图片内联 blob 链接、文本内联引用）

人工验收                reviewer 查看 PR 证据评论，对照清单逐项核实后，
                        直接在 PR body 点击 checkbox 打勾

门禁                    - 软门禁：daemon 轮询 agent/review Issue，未全勾维持
                          validation/pending；全勾且 head 与证据一致换
                          validation/passed 并写 validation_passed 审计 comment；
                          勾选后又有新 push → 重置勾选并写 validation_reset comment
                        - 硬门禁：.github/workflows/validation-gate.yml 解析
                          PR body 区块，未全勾 check 失败

Issue 关闭              daemon 自动删除 iar-evidence/issue-<N> 分支
```
