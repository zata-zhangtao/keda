...
- 仓库级 `labels`、`git`、`worktree`、`runner`、`safety` 可覆盖全局 `[agent_runner]` 默认值。
- 未指定 `--repo` 或 `--repo-id` 时，`labels sync`、`run-once`、`daemon` 会自动处理所有 `enabled = true` 的仓库。
...
- `core/use_cases/` — 业务用例（`sync_labels`、`create_issue_from_prd`、`run_agent_once`、`run_agent_repositories_once`、`run_agent_daemon`）
