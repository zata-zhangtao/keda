...
def test_provision_worktree_database_runs_setup_and_migration(tmp_path: Path) -> None:
    ...
    worktree_path = repository_path / ".iar-worktrees" / "issue-42"
...
def test_provision_worktree_database_rejects_missing_template_script(tmp_path: Path) -> None:
    """Enabled provisioning fails fast when a repository lacks the synced helper."""
    repository_path = tmp_path / "keda"
    worktree_path = repository_path / ".iar-worktrees" / "issue-7"
    worktree_path.mkdir(parents=True)

    with pytest.raises(FileNotFoundError, match="database setup script"):
        provision_worktree_database(
